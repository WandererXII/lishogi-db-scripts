...
case class KifStd(
  ...
  def move(situation: Situation): Validated[String, oldShogi.Move] =
...
case class PGNStd(
  ...
  def move(situation: Situation): Validated[String, oldShogi.Move] =
    situation.board.pieces.foldLeft(none[oldShogi.Move]) {
      case (None, (pos, piece))
...
case class Drop(
  ...
  def drop(situation: Situation): Validated[String, oldShogi.Drop] =

...
function encodePath(path) {
  return path
    ? path
        .replace(dotRegex, String.fromCharCode(251))
        .replace(dollarRegex, String.fromCharCode(252))
    : "";
}
...
const totalNb =
  (db.study_chapter.count() - db.study_chapter_flat.count()) / parallelism;
    ...
    if (i % batchSize == 0) {
      ...
      print(
        `${i} ${percent.toFixed(
          ...
          sumMoves / batchSize
        ).toFixed(0)} big:${tooBigNb} deep:${tooDeepNb}`
      );
      lastAt = at;
      sumSizeFrom = 0;
      sumSizeTo = 0;
